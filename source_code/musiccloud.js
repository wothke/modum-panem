...
Filters = {
	/*
		...
		ALLPASS, ...
	*/	
	LOWPASS: ("LOWPASS" in f) ? f.LOWPASS : 'lowpass',
	HIGHPASS: ("HIGHPASS" in f) ? f.HIGHPASS : 'highpass',
	BANDPASS: ("BANDPASS" in f) ? f.BANDPASS : 'bandpass',
	LOWSHELF: ("LOWSHELF" in f) ? f.LOWSHELF : 'lowshelf',
	PEAKING: ("PEAKING" in f) ? f.PEAKING : 'peaking',
	HIGHSHELF: ("HIGHSHELF" in f) ? f.HIGHSHELF : 'highshelf',
	NOTCH: ("NOTCH" in f) ? f.NOTCH : 'notch',
	ALLPASS: ("ALLPASS" in f) ? f.ALLPASS : 'allpass',
...
//		var permLink= song.permalink_url.replace("http:\/\/soundcloud.com", "");
		var permLink= song.permalink_url.replace("http:\/\/", "https:\/\/");
		

		var tit= $(this.guiIdMap.title);
		tit.html('<a target="_blank" href="'+permLink+'">'+song.title +'</a>');
	...
	initMusic: function() {
		...
		var cfg= [	
					[32, Filters.HIGHPASS, 19000, 0, -40, null],	// hihat
					[32, Filters.BANDPASS, 9900, 100, 0, null],	// band
					[32, Filters.LOWPASS, 5000, 0, -40, null]		// vol
				];
		this.startMusic(cfg);
	},
